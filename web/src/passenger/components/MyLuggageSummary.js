  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base font-bold text-gray-900">My Luggage Reports</h2>
        <Link
          to="/passenger/report"
          className="bg-blue-600 text-white px-3 py-1 rounded font-semibold text-sm hover:bg-blue-700 transition-colors"
        >
          Report Lost
        </Link>
      </div>
        ...
        {cases.map((lugCase) => (
          <div key={lugCase.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
            <div className="flex items-start justify-between">
              ...
              <Link to={`/passenger/cases/${lugCase.id}`} className="text-blue-600 hover:text-blue-800 font-medium text-sm">
                View
              </Link>
            </div>
          </div>
